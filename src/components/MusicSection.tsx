...
import { Play, Pause, Volume2, BookOpen, Clock, Music, X, Heart, ExternalLink, SkipBack, SkipForward } from 'lucide-react';
...
export default function MusicSection() {
  ...
  return (
    <section id="music" className="bg-base py-32 text-text-main relative overflow-hidden">
      {/* Decorative ambient visualizer shape */}
      ...
      <div className="max-w-5xl mx-auto px-6 lg:px-12 relative z-10">
              ...
              >
                <div className="relative aspect-[4/3] overflow-hidden bg-surface">
                  <img 
                    src={release.cover} 
                    alt={release.title} 
                    className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-[1.5s] opacity-80 group-hover:opacity-100 ease-out"
                  />
        ...
        {/* Master Active Audio Player bar */}
        <audio 
          ref={audioRef}
          src={audioUrl}
          ...
          {activeTrack && activeRelease && (
            ...
            >
              <button 
                ...
                className="absolute top-4 right-4 text-text-muted/50 hover:text-accent transition-colors p-1"
                title="Close Player"
              >
                <X size={16} />
              </button>
              ...
              <div className="flex flex-col lg:flex-row items-center gap-8 justify-between">
                ...
                <div className="flex items-center gap-4 w-full lg:w-auto">
                  <div className="w-16 h-16 rounded overflow-hidden flex-shrink-0 border border-white/5 relative bg-base">
                    <img src={activeRelease.cover} alt={activeRelease.title} className="w-full h-full object-cover" />
                    {isPlaying && (
                      <div className="absolute inset-0 flex items-center justify-center bg-base/60">
                        <span className="flex gap-1 items-end h-5">
                          ...
                          <span className="w-0.5 bg-accent h-3 animate-pulse delay-75 animate-bounce-slow" />
                          <span className="w-0.5 bg-accent h-4 animate-pulse delay-150" />
                        </span>
                      </div>
                    )}
                  </div>
                  <div>
                    <h4 className="text-xl font-display font-semibold tracking-tight text-text-main">{activeRelease.title}</h4>
                    <p className="text-text-muted text-[11px] uppercase tracking-widest">{activeRelease.subtitle}</p>
                  </div>
                </div>

                {/* Simulated Waveform & Controls */}
                <div className="flex-1 max-w-md w-full">
                  <div className="flex items-center gap-4 justify-center mb-3">
                    <button 
                      onClick={() => handleSkip(-15)}
                      className="text-text-muted hover:text-accent transition-colors flex items-center justify-center w-8 h-8 focus:outline-none"
                      title="Skip back 15s"
                    >
                      <SkipBack size={16} />
                    </button>
                    <button 
                      ...
                      className="w-10 h-10 rounded-full bg-accent/10 border border-accent/20 hover:bg-accent hover:border-accent text-accent hover:text-base flex items-center justify-center transition-all focus:outline-none"
                    >
                    ...
                    {/* Live Scrolling Subtitle Lyric Line */}
                    <div className="text-center font-serif text-sm italic text-accent min-h-[20px] transition-all duration-300">
                      {currentLyricLine ? (
                  ...
                  {/* Progress tracker */}
                  <div 
                    className="relative w-full h-1 bg-base rounded-full overflow-hidden cursor-pointer"
                    onClick={handleSeek}
                  >
                    <motion.div 
                  ...
                  <div className="flex justify-between items-center text-[10px] text-text-muted mt-2 font-mono">
                    ...
                    <span className="flex items-center gap-1"><Heart size={10} className="fill-accent stroke-accent" /> Playback Active</span>
                  </div>
                </div>

                {/* Volume & Lyrics action buttons */}
                <div className="flex items-center gap-6 w-full lg:w-auto justify-end">
                  ...
                  >
                    <BookOpen size={12} /> Discography Info
                  </button>
                </div>
              </div>

              {/* Expandable lyric sheets + discography notes */}
              <AnimatePresence>
                {showLyrics && (
                  <motion.div
                    initial={{ height: 0, opacity: 0 }}
