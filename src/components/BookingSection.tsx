  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setErrMsg(
        e instanceof Error
          ? e.message
          : "We couldn't submit your inquiry right now. Please email booking@zacharywalkermusic.com.",
      );
    }
  };
            ...
            {status === 'success' ? (
              ...
              >
                <Field label="Your Name *">
                  <input type="text" value={form.name} onChange={(e) => update('name', e.target.value)} className={inputCls} required autoComplete="name" />
                </Field>
                <Field label="Email *">
                  <input type="email" value={form.email} onChange={(e) => update('email', e.target.value)} className={inputCls} required autoComplete="email" />
                </Field>
                <Field label="Phone (optional)">
                  <input type="tel" value={form.phone} onChange={(e) => update('phone', e.target.value)} className={inputCls} autoComplete="tel" />
                </Field>
                ...
                <Field label="Event Date *">
                  <input type="date" value={form.eventDate} onChange={(e) => update('eventDate', e.target.value)} className={inputCls} required />
                </Field>
                <Field label="Set Length / Hours">
                  <input type="text" placeholder="e.g. 3 hours, 6:30pm–9:30pm" value={form.hours} onChange={(e) => update('hours', e.target.value)} className={inputCls} />
                </Field>
                <Field label="Venue Name">
                  <input type="text" value={form.venue} onChange={(e) => update('venue', e.target.value)} className={inputCls} />
                </Field>
                ...
                <Field label="Budget Range (optional)" full>
                  <input type="text" placeholder="Helps tailor a fair quote" value={form.budget} onChange={(e) => update('budget', e.target.value)} className={inputCls} />
                </Field>
                ...
                <div className="md:col-span-2 flex items-start gap-3 text-xs text-text-muted">
                  ...
                  <label htmlFor="consent" className="leading-relaxed">
                    I agree to be contacted about this inquiry and acknowledge the Privacy Policy.
                    Submitted details are stored solely for the purpose of responding to this booking request.
                  </label>
                </div>
          ...
          <aside className="flex flex-col gap-6">
            <div className="bg-surface/60 backdrop-blur-md border border-white/5 rounded-2xl p-6 md:p-8">
              ...
              <ul className="space-y-3 text-sm text-text-muted">
                ...
                <Bullet>Self-contained PA suitable for up to 150 guests</Bullet>
            ...
            <div className="bg-accent/5 border border-accent/30 rounded-2xl p-6">
              ...
              <p className="text-text-muted text-xs leading-relaxed mb-4">
                Not ready to fill out the full form? Email directly and I&apos;ll reply personally.
              </p>
              <a
                href="mailto:booking@zacharywalkermusic.com"
